...
const StyledHeaderOne = styled.div`
	p {
		text-shadow: ${(props) => (props.loading ? '0 0 20px ' + Colors.manifestBlue : 'none')};
		font-family: roboto;
		font-style: normal;
		font-weight: bold;
		font-size: 36px;
...
const HeaderTwo = ({ text, bold, color = 'black', onClick, pointer = false }) => {
	return (
		<StyledHeaderTwo pointer={pointer} onClick={onClick} bold={bold} color={color}>
			{text}
		</StyledHeaderTwo>
	);
};
...
const Description = ({ text, color = 'black', onClick, bold = false, pointer = false, maxSize = 0 }) => {
	const getText = () => {
	...
	return (
		<StyledDescription onClick={onClick} color={color} bold={bold} pointer={pointer}>
			<p>{getText()}</p>
		</StyledDescription>
	);
};

const InfoTitle = ({ text, color = 'black', onClick, bold = false, pointer = false }) => {
	return (
		<StyledInfoTitle onClick={onClick} color={color} bold={bold} pointer={pointer}>
			{text}
		</StyledInfoTitle>
	);
};

const Note = ({ text, color = 'black', bold = false, onClick, pointer = false, maxSize }) => {
	const getText = () => {
		if (maxSize && !isEmpty(text)) {
			if (text.length > maxSize) {
				return text.slice(0, maxSize - 8) + '...';
...
const FinePrint = ({ text, color = Colors.manifestBlue, pointer = false, bold, maxSize }) => {
	const getText = () => {
		if (maxSize && !isEmpty(text)) {
			if (text.length > maxSize) {
				return text.slice(0, maxSize - 8) + '...';
